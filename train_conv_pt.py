...
net = nn.Sequential(nn.Conv2d(1, 1024, 2), 
                    nn.ReLU(),
...
def trainNetwork(model):
    ...
    while(1):
        ...
        state = torch.tensor(s_t).to(device)
        readout_t = model(state.view(1,1,4,4).float())
        ...
        if t > OBSERVE:
            ...
            target = model(state_batch_torch.view(BATCH,1,4,4).float().to(device))
            ...
            readout_j1_batch = model(next_state_batch_torch.view(BATCH,1,4,4).float().to(device))
            ...
            for i in range(0, len(minibatch)):
                ...
                if termi:
                    ...
                    target[i][np.argmax(a_batch[i])] = r_batch[i] + GAMMA * (np.max(readout_j1_batch[i].detach().cpu().numpy()))

            # perform gradient step
            #state_batch = np.array(s_j_batch).astype('float32').reshape(BATCH,4,4,1)
            #model.train_on_batch(state_batch,target)

            loss = F.smooth_l1_loss(readout_j1_batch.to(device), target.to(device) ) 
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        print("TIMESTEP", t, "/ STATE", state, \
            ...
            "/ Q_MAX %e" % np.max(readout_t.detach().cpu().numpy()))
